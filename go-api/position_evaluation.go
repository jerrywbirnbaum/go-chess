...
func getEvalCell(evalTable [64]int, row int, col int, flip bool) int {
	idx := row*8 + col
	if flip {
		idx ^= 56
	}

	return evalTable[idx]

}

var mg_value = [6]int{82, 337, 365, 477, 1025, 0}
var eg_value = [6]int{94, 281, 297, 512, 936, 0}
...
var mg_table [12][64]int
var eg_table [12][64]int
	...
	for i := range 13 {
		...
		for row := range 8 {
			for col := range 8 {
				...
				mg_table[piece-1][idx] = getEvalCell(mg_pesto_table[pieceType-1], row, col, flip) + mg_value[pieceType-1]
				eg_table[piece-1][idx] = getEvalCell(eg_pesto_table[pieceType-1], row, col, flip) + eg_value[pieceType-1]
				idx += 1
			}
		}
	}
...
func pestoEval(b *Board) int {
	...
	midGamePhase := min(b.midGamePhase, 24)
